...
M.generalDriver = function()
  ...
  if args.subj_data.run_single_subj then
    ...
    subj_data = sleep_eeg.CVBySubjData(args.subj_data.filename, 
      args.subj_data.do_kfold_split, nil, args.subj_data.percent_train, 
      args.subj_data.predict_subj, args.subj_data.num_folds, 
      args.subj_data.fold_num)
  end
  ...
  if args.subj_data.wake then
    ...
    --make a closure that will pass in the 'test' arg to subsetConfusionMatrix
    local testConfSubsetMatrix = function(state)
      sleep_eeg.hooks.subsetConfusionMatrix(state, 'test', subj_data.classnames, {1,2}, args.subj_data.predict_subj and 1 or nil)
    end
    table.insert(args.training.trainingIterationHooks, testConfSubsetMatrix)
